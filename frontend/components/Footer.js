...
(function () {
   ...
   * This allows the same script to work on /index.html and /about/us.html
   */
  function getRootPrefix() {
    ...
    // Calculate how many levels deep we are relative to the 'frontend' or root directory
    // Assuming the structure is: /project/frontend/pages/...
    // If we are at /index.html, depth is 0. If at /academics/faculty.html, depth is 1.
    var prefix = '';
  ...
  /**
   * Mock or real contact data fetch
   */
  ...
  /* ── CSS: Unified styles with CSS variables for easy theming ── */
  var FOOTER_CSS = `
<style id="lcs-footer-styles">
  ...
  .ft-top-bar {
    ...
    padding: 10px 48px;
    border-bottom: 1px solid var(--ft-gold-glow);
  }
  .ft-topbar-item {
    display: flex;
    align-items: center;
  ...
  .ft-topbar-item a:hover { color: var(--ft-gold-light); }
  ...
  .ft-logo-ring { 
    width: 54px; height: 54px; border-radius: 50%; border: 2px solid var(--ft-gold); 
    overflow: hidden; flex-shrink: 0; background: #0a3535; display: flex; align-items: center; 
    justify-content: center; font-family: Georgia, serif; font-size: 14px; font-weight: 700; 
    color: var(--ft-gold); letter-spacing: 1px;
  }
  ...
  .ft-soc-btn { 
    ...
    justify-content: center; color: var(--ft-text-dim); text-decoration: none; transition: all 0.2s; cursor: pointer; 
  }
  ...
  .ft-col-heading { 
    ...
    color: var(--ft-gold); margin-bottom: 20px; display: flex; align-items: center; gap: 8px; 
  }
  ...
  .ft-links li a { 
    display: flex; align-items: center; gap: 8px; font-size: 0.82rem; color: var(--ft-text-dim); 
    text-decoration: none; padding: 5px 0; transition: all 0.18s; 
  }
  ...
  .ft-contact-icon { 
    width: 32px; height: 32px; border-radius: 8px; background: var(--ft-gold-glow); 
    border: 1px solid rgba(200, 150, 12, 0.2); display: flex; align-items: center; justify-content: center; flex-shrink: 0; 
  }
  ...
  .ft-login-btn { 
    ...
    padding: 10px; background: var(--ft-gold-glow); border: 1px solid rgba(200,150,12,0.3); 
    border-radius: 10px; font-size: 0.78rem; font-weight: 700; color: var(--ft-gold-light); 
  ...
  .ft-bottom-links a:hover { color: var(--ft-gold); }
  ...
  .ft-credits strong { color: rgba(255,255,255,0.5); }
  
  /* ADMIN DOT - THE FUNCTIONAL TRIGGER */
  .ft-admin-trigger { 
    display: inline-block; width: 6px; height: 6px; border-radius: 50%; background: rgba(255,255,255,0.08); 
    margin-left: 8px; vertical-align: middle; transition: all 0.3s; cursor: pointer; text-decoration: none; border: none;
  }
  ...
  @media (max-width: 768px) {
    ...
    .mob-ft-card { 
      background: #083838; border: 1px solid rgba(200,150,12,0.35); border-radius: 20px; 
      padding: 20px 18px 16px; position: relative; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.3);
    }
    ...
    .mob-ft-credits { padding:14px 0 4px; text-align:center; font-family:'Nunito',sans-serif; font-size:0.65rem; text-transform:uppercase; letter-spacing:0.1em; color:rgba(255,255,255,0.3); line-height:1.6; }
    .mob-ft-credits a { color:#e8b020; text-decoration:none; font-weight:800; }
    .mob-ft-credits strong { color:rgba(255,255,255,0.55); font-weight:800; }
  }

  /* Scroll Top Button */
  #ft-scroll-top {
    position: fixed; bottom: 30px; right: 30px; width: 45px; height: 45px;
    background: var(--ft-gold); color: var(--ft-bg); border: none; border-radius: 50%;
  ...
  #ft-scroll-top:hover { background: var(--ft-gold-light); transform: translateY(-5px); }
</style>
`;
  ...
  var FOOTER_HTML = [
    ...
    '          </div>',
    '        </a>',
    '        <p class="ft-brand-desc">Shaping young minds for over three decades with excellence in education, rooted in values, innovation, and care.</p>',
    '        <div class="ft-social">',
    ...
    '          <a class="ft-soc-btn" aria-label="Twitter/X" href="#"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>',
    ...
    '    <div class="ft-bottom">',
    '      <span class="ft-copy">© <span class="ft-year"></span> Loretto Central School, Bantwal. All rights reserved. &nbsp;·&nbsp; CBSE Affiliation No. 831368</span>',
    ...
    '          <a href="' + R + 'news.html" class="mob-ft-link">News &amp; Events</a>',
  ...
  async function injectHTML() {
    ...
    // Dynamic Contact Updates (if any)
    ...
    // Functional Admin Trigger (Secret: Triple click the logo to go to admin)
    ...
    // Also make the dot functional for mobile if hidden
    var dot = document.getElementById('ft-admin-trigger');
    if (dot) {
      // Functional link is already set in href
    }
  }
  ...
  // Global access for re-init if needed

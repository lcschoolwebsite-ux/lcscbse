(function () {
  ...
  async function initFormerHeadsPage() {
    if (!/\/academics\/2-former-heads\.html(?:$|\?)/.test(window.location.pathname)) return;
  ...
  async function initBookListPage() {
    if (!/\/academics\/5-book-list\.html(?:$|\?)/.test(window.location.pathname)) return;
  ...
  async function initSchoolCircularsPage() {
    if (!/\/academics\/4-school-circulars\.html(?:$|\?)/.test(window.location.pathname)) return;
